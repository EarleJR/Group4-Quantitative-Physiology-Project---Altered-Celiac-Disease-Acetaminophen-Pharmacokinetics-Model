...
% 5 = run two simulations (softgel, celiac disease softgel)
...
%         SaveFigs = 0;    % set to 0 if we don't want to save figures every loop   
    ...
    case 5
        ...
        V1x = 5000; % mL
        V2x = 5000; % mL
        CA0 = 50; % ug/ml (will be * ml = ug); 250mg; 13.7 originally for gel
        kA =  .764; % hr-1
        
        OralOrIV = 1; % 1 = oral; 2 = IV
        [AUC1(i),AUC2(i)] = Acet_sim(q0,CA0,kA,V1x,V2x,OralOrIV,VisibleFigs,SaveFigs,"Softgel");

        i=2; % Softgel formulation with celiac disease
%       These values are unchanged for tablet formulation    
%         q0 = 0; % ug/hr
%         V1x = 5000; % mL
%         V2x = 5000; % mi
        CA0 = 50; % ug/ml * ml = ug; 250mg
        ge_celiac = 45.2; % GE(t1/2); units = min
        ge_control = 23.6; % GE(t1/2); units = min
        kA = (ge_control/ge_celiac)*(.764); % hr-1

        OralOrIV = 1; % 1 = oral; 2 = IV
        [AUC1(i),AUC2(i)] = Acet_sim(q0,CA0,kA,V1x,V2x,OralOrIV,VisibleFigs,SaveFigs,"Softgel with Celiac Disease");
        ...
        ylabel(gca,'AUC (ug*hr/ml)')
        xlabel(gca,'Scenario')
        set(gca,'XTickLabel',{'Softgel','Softgel with Celiac'});
    ...
    case 6
        ...
        i=2; % Softgel formulation with severe celiac disease
%       These values are unchanged for tablet formulation    
%         q0 = 0; % ug/hr
%         V1x = 5000; % mL
%         V2x = 5000; % mi
        CA0 = 50; % ug/ml * ml = ug; 250mg
        ge_celiac = 60.1; % GE(t1/2); units = min
        ge_control = 13.3; % GE(t1/2); units = min
        kA = (ge_control/ge_celiac)*(.764); % hr-1

        OralOrIV = 1; % 1 = oral; 2 = IV
        [AUC1(i),AUC2(i)] = Acet_sim(q0,CA0,kA,V1x,V2x,OralOrIV,VisibleFigs,SaveFigs,"Softgel with Severe Celiac Disease");
        ...
        ylabel(gca,'AUC (ug*hr/ml)')
        xlabel(gca,'Scenario')
        set(gca,'XTickLabel',{'Softgel','Softgel with Severe Celiac'});
